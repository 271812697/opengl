...
Rendering::Context::Driver::Driver(const Settings::DriverSettings& p_driverSettings)
{
	InitGlad();
...
bool Rendering::Context::Driver::IsActive() const
{
	return m_isActive;
}

void Rendering::Context::Driver::GLDebugMessageCallback(uint32_t source, uint32_t type, uint32_t id, uint32_t severity, int32_t length, const char* message, const void* userParam)
{
...
void Rendering::Context::Driver::InitGlad()
{
