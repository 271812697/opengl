...
uint32_t Pack(const Maths::FVector3& p_toPack)
{
...
Rendering::Entities::Light::Light(Maths::FTransform & p_tranform, Type p_type) : m_transform(p_tranform), type(static_cast<float>(p_type))
{
	
}

Maths::FMatrix4 Rendering::Entities::Light::GenerateMatrix() const
{
...
float CalculateAmbientBoxLightRadius(const Maths::FVector3& p_position, const Maths::FVector3& p_size)
{
	return Maths::FVector3::Distance(p_position, p_position + p_size);
}

float Rendering::Entities::Light::GetEffectRange() const
{
	switch (static_cast<Rendering::Entities::Light::Type>(static_cast<int>(type)))
	{
	case Type::POINT:
...
const Maths::FTransform& Rendering::Entities::Light::GetTransform() const
{
	return m_transform;
}
