...
void Core::ECS::Components::CAudioSource::OnInspector(UI::Internal::WidgetContainer& p_root)
{
	using namespace Audio::Entities;
	using namespace Core::Helpers;
	...
	auto& graph = p_root.CreateWidget<UI::Widgets::Plots::PlotLines>(std::vector<float>(), -0.1f, 1.1f);

	graph.size.y = 75.0f;

	auto& graphDispacther = graph.AddPlugin<UI::Plugins::DataDispatcher<std::vector<float>>>();
	...
	{ 
		...
		Maths::FVector3 listenerPosition(0.0f, 0.0f, 0.0f);
		bool playMode = Core::Global::ServiceLocator::Get<SceneSystem::SceneManager>().GetCurrentScene()->IsPlaying();
		auto listenerInfo = Core::Global::ServiceLocator::Get<Audio::Core::AudioEngine>().GetListenerInformation(!playMode);
		if (listenerInfo.has_value())
			listenerPosition = listenerInfo.value().first;

		float distanceToListener = Maths::FVector3::Distance(listenerPosition, owner.transform.GetWorldPosition());
