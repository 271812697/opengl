...
void Core::Scripting::LuaGlobalsBinder::BindGlobals(sol::state & p_luaState)
{
	...
	auto CreateActorerload = sol::overload
	(
	...
	p_luaState["Inputs"]["GetKeyDown"] = [](EKey p_key) { return OVSERVICE(InputManager).IsKeyPressed(p_key); };
	p_luaState["Inputs"]["GetKeyUp"] = [](EKey p_key) { return OVSERVICE(InputManager).IsKeyReleased(p_key); };
	p_luaState["Inputs"]["GetKey"] = [](EKey p_key) { return OVSERVICE(InputManager).GetKeyState(p_key) == EKeyState::KEY_DOWN; };

	p_luaState["Inputs"]["GetMouseButtonDown"] = [](EMouseButton p_button) { return OVSERVICE(InputManager).IsMouseButtonPressed(p_button); };
	p_luaState["Inputs"]["GetMouseButtonUp"] = [](EMouseButton p_button) { return OVSERVICE(InputManager).IsMouseButtonReleased(p_button); };
	p_luaState["Inputs"]["GetMouseButton"] = [](EMouseButton p_button) { return OVSERVICE(InputManager).GetMouseButtonState(p_button) == EMouseButtonState::MOUSE_DOWN; };

	p_luaState["Inputs"]["GetMousePos"] = []()
	{
		auto mousePos = OVSERVICE(InputManager).GetMousePosition();
		return FVector3(static_cast<float>(mousePos.first), static_cast<float>(mousePos.second));
	};

	p_luaState["Inputs"]["LockMouse"] = []() { return OVSERVICE(Window).SetCursorMode(Cursor::ECursorMode::DISABLED); };
	p_luaState["Inputs"]["UnlockMouse"] = []() { return OVSERVICE(Window).SetCursorMode(Cursor::ECursorMode::NORMAL); };

	p_luaState["Scenes"]["GetCurrentScene"] = []() -> Scene& { return *OVSERVICE(SceneManager).GetCurrentScene(); };
	p_luaState["Scenes"]["Load"] = [](const std::string& p_path)
	{ 
		OVSERVICE(SceneManager).LoadAndPlayDelayed(p_path);
	};

	p_luaState["Resources"]["GetModel"] = [](const std::string& p_resPath) { return OVSERVICE(ModelManager).GetResource(p_resPath); };
	p_luaState["Resources"]["GetShader"] = [](const std::string& p_resPath) { return OVSERVICE(ShaderManager).GetResource(p_resPath); };
	p_luaState["Resources"]["GetTexture"] = [](const std::string& p_resPath) { return OVSERVICE(TextureManager).GetResource(p_resPath); };
	p_luaState["Resources"]["GetMaterial"] = [](const std::string& p_resPath) { return OVSERVICE(MaterialManager).GetResource(p_resPath); };
	p_luaState["Resources"]["GetSound"] = [](const std::string& p_resPath) { return OVSERVICE(SoundManager).GetResource(p_resPath); };

	p_luaState["Math"]["RandomInt"] = [](int p_min, int p_max) { return Tools::Utils::Random::Generate(p_min, p_max); };
	p_luaState["Math"]["RandomFloat"] = [](float p_min, float p_max) { return Tools::Utils::Random::Generate(p_min, p_max); };
	p_luaState["Math"]["CheckPercentage"] = [](float p_percentage) { return Tools::Utils::Random::CheckPercentage(p_percentage); };
	...
	p_luaState["Physics"]["Raycast"] = [](const Maths::FVector3& p_origin, const Maths::FVector3& p_direction, float p_distance) -> std::optional<PhysicsWrapper::RaycastHit>
	{
