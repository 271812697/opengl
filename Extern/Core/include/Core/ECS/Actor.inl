...
namespace Core::ECS
{
	...
	inline T& Core::ECS::Actor::AddComponent(Args&& ...p_args)
	{
		...
		if (auto found = GetComponent<T>(); !found)
		{
			...
			{
				reinterpret_cast<Core::ECS::Components::AComponent&>(instance).OnAwake();
				reinterpret_cast<Core::ECS::Components::AComponent&>(instance).OnEnable();
				reinterpret_cast<Core::ECS::Components::AComponent&>(instance).OnStart();
			}
			return instance;
		}
		else
	...
	template<typename T>
	inline bool Core::ECS::Actor::RemoveComponent()
	{
		static_assert(std::is_base_of<Components::AComponent, T>::value, "T should derive from AComponent");
