...
std::map< std::pair<PhysicalObject*, PhysicalObject*>, bool> Physics::Core::PhysicsEngine::m_collisionEvents;

Physics::Core::PhysicsEngine::PhysicsEngine(const Settings::PhysicsSettings & p_settings)
{
...
void Physics::Core::PhysicsEngine::PreUpdate()
{
...
void Physics::Core::PhysicsEngine::PostUpdate()
{
...
bool Physics::Core::PhysicsEngine::Update(float p_deltaTime)
{
	PreUpdate();
...
std::optional<RaycastHit> Physics::Core::PhysicsEngine::Raycast(Maths::FVector3 p_origin, Maths::FVector3 p_direction, float p_distance)
{
	if (p_direction == Maths::FVector3::Zero)
		return {};
	...
	{
		// Get First Hit
		resultHit.FirstResultObject = reinterpret_cast<Physics::Entities::PhysicalObject*>(ClosestRayCallback.m_collisionObject->getUserPointer());
		...
		for (int i = 0; i < rayCallback.m_collisionObjects.size(); i++)
			resultHit.ResultObjects.push_back(reinterpret_cast<Physics::Entities::PhysicalObject*>(rayCallback.m_collisionObjects[i]->getUserPointer()));
...
void Physics::Core::PhysicsEngine::SetGravity(const Maths::FVector3 & p_gravity)
{
	m_world->setGravity(Conversion::ToBtVector3(p_gravity));
}

Maths::FVector3 Physics::Core::PhysicsEngine::GetGravity() const
{
	return Conversion::ToVector3(m_world->getGravity());
}

void Physics::Core::PhysicsEngine::ListenToPhysicalObjects()
{
...
void Physics::Core::PhysicsEngine::Consider(PhysicalObject& p_toConsider)
{
	m_physicalObjects.push_back(std::ref(p_toConsider));
}

void Physics::Core::PhysicsEngine::Unconsider(PhysicalObject& p_toUnconsider)
{
	{
...
void Physics::Core::PhysicsEngine::Consider(btRigidBody& p_toConsider)
{
	m_world->addRigidBody(&p_toConsider);
}

void Physics::Core::PhysicsEngine::Unconsider(btRigidBody& p_toUnconsider)
{
	m_world->removeRigidBody(&p_toUnconsider);
}

void Physics::Core::PhysicsEngine::ResetCollisionEvents()
{
	for (auto& element : m_collisionEvents)
		element.second = false;
}

void Physics::Core::PhysicsEngine::CheckCollisionStopEvents()
{
...
bool Physics::Core::PhysicsEngine::CollisionCallback(btManifoldPoint& cp, const btCollisionObjectWrapper* obj1, int id1, int index1, const btCollisionObjectWrapper* obj2, int id2, int index2)
{
...
void Physics::Core::PhysicsEngine::SetCollisionCallback()
{
