...
namespace Geometry
{
	...
	inline void Gennormal(MeshData<VertexPosNormalColorTex>& meshData) {
		...
		for (int i = 0; 3 * i + 2 < meshData.indexVec.size(); i++) {
			int a = meshData.indexVec[3 * i], b = meshData.indexVec[3 * i + 1], c =meshData.indexVec[ 3 * i + 2];
			DirectX::XMVECTOR p1 = { meshData.vertexVec[b].pos.x - meshData.vertexVec[a].pos.x,
			                         meshData.vertexVec[b].pos.y - meshData.vertexVec[a].pos.y ,
				                     meshData.vertexVec[b].pos.z - meshData.vertexVec[a].pos.z };
			DirectX::XMVECTOR p2 = { meshData.vertexVec[c].pos.x - meshData.vertexVec[a].pos.x,
						             meshData.vertexVec[c].pos.y - meshData.vertexVec[a].pos.y ,
						             meshData.vertexVec[c].pos.z - meshData.vertexVec[a].pos.z };
			auto N= DirectX::XMVector3Normalize(DirectX::XMVector3Cross(p1,p2));
			DirectX::XMFLOAT3 p;
			DirectX::XMStoreFloat3(&p, N);
			
			meshData.vertexVec[a].normal = { meshData.vertexVec[a].normal.x + p.x,meshData.vertexVec[a].normal.y + p.y,meshData.vertexVec[a].normal.z + p.z };
			meshData.vertexVec[b].normal = { meshData.vertexVec[b].normal.x + p.x,meshData.vertexVec[b].normal.y + p.y,meshData.vertexVec[b].normal.z + p.z };
			meshData.vertexVec[c].normal = { meshData.vertexVec[c].normal.x + p.x,meshData.vertexVec[c].normal.y + p.y,meshData.vertexVec[c].normal.z + p.z };
			cnt[a]++;
			cnt[b]++;
			cnt[c]++;
		}
		for (int i = 0; i < meshData.vertexVec.size(); i++) {
			...
			meshData.vertexVec[i].normal = { meshData.vertexVec[i].normal.x / cnt[i] ,meshData.vertexVec[i].normal.y / cnt[i], meshData.vertexVec[i].normal.z / cnt[i] };
	...
	inline std::vector<Geometry::MeshData<VertexPosColor>>loadSphereFromFile(const char* path = nullptr,float r=0.015) {
		std::ifstream f;
		f.open(path);
		std::vector<Geometry::MeshData<VertexPosColor>> ans;
		int num;
		f >>num;
		...
		for (int i = 0; i < num; i++) {
			f >> x >> y >> z ;
			meshData = CreateSphere<VertexPosColor>(r, 20, 20, {1.0f,1.0f,0.0f,1.0f});
			for (auto& it : meshData.vertexVec) {
                  it.pos = DirectX::XMFLOAT3{x+ it.pos.x,y+ it.pos.y,z+ it.pos.z};
		     }
			ans.push_back(meshData);
		}
		f.close();
		return ans;     
	}
	...
	inline std::pair<MeshData<VertexPosNormalColorTex>, std::vector<std::pair<DirectX::XMFLOAT3, DirectX::XMFLOAT3>>>loadGroundTruth(const char* path = nullptr) {
		...
		int num;
		f >> num;
		std::vector<std::pair<DirectX::XMFLOAT3, DirectX::XMFLOAT3>> poi;
		for (int i = 0; i < num; i++) {
			f >> x >> y >> z;
			poi.push_back(std::make_pair(DirectX::XMFLOAT3{ x ,y ,z }, DirectX::XMFLOAT3{ 1 ,1,0 }));
		}
		f.close();
		Gennormal(meshData);
	...
	inline void loadFromFile(MeshData<VertexPosNormalColorTex>* meshData, std::vector<std::pair<DirectX::XMFLOAT3, Geometry::MeshData<VertexPosNormalColorTex>>>* poi, const char* path = nullptr) {
		...
		for (int i = 0; i < num; i++) {
			...
			poi->push_back(std::make_pair (DirectX::XMFLOAT3{x,y,z},mesh ) );
		}
		f.close();
	}
	...
	inline void StoreToFile(std::vector<std::pair<DirectX::XMFLOAT3, DirectX::XMFLOAT3>>pos, const char* path = nullptr) {
		std::ofstream f;
		f.open(path, std::ios::out);
		//先读取模型
		f << pos.size() << "\n";
