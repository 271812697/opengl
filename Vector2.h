...
public:
    ...
    float AbsDotProduct(const Vector2& rhs) const { return Opengl::Abs(x_ * rhs.x_) +Opengl::Abs(y_ * rhs.y_); }

    /// Project vector onto axis.
    float ProjectOntoAxis(const Vector2& axis) const { return DotProduct(axis.Normalized()); }

    /// Returns the angle between this vector and another vector in degrees.
    float Angle(const Vector2& rhs) const { return Opengl::Acos(DotProduct(rhs) / (Length() * rhs.Length())); }

    /// Return absolute vector.
    Vector2 Abs() const { return Vector2(Opengl::Abs(x_), Opengl::Abs(y_)); }

    /// Linear interpolation with another vector.
    Vector2 Lerp(const Vector2& rhs, float t) const { return *this * (1.0f - t) + rhs * t; }

    /// Test for equality with another vector with epsilon.
    bool Equals(const Vector2& rhs) const { return x_==rhs.x_&& y_==rhs.y_; }
    ...
    Vector2 NormalizedOrDefault(const Vector2& defaultValue = Vector2::ZERO, float eps =Opengl::M_LARGE_EPSILON) const
    {
        const float lenSquared = LengthSquared();
    ...
    Vector2 ReNormalized(float minLength, float maxLength, const Vector2& defaultValue = Vector2::ZERO, float eps = Opengl::M_LARGE_EPSILON) const
    {
        const float lenSquared = LengthSquared();
        if (lenSquared < eps * eps)
            return defaultValue;
        ...
        const float newLen =Opengl::Clamp(len, minLength, maxLength);
