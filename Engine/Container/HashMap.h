...
public:
    ...
    HashMap()
    {
        ...
        allocator_ = AllocatorInitialize((int)sizeof(Node));
    ...
    HashMap(const HashMap<T, U>& map)
    {
        ...
        allocator_ = AllocatorInitialize((int)sizeof(Node), map.Size() + 1);
        head_ = tail_ = ReserveNode();
        *this = map;
    }
    ...
    bool Rehash(int numBuckets)
    {
        ...
        if (!!(numBuckets & (numBuckets - 1)) && numBuckets)
            return false;
