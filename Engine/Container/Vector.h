...
/// No index.
inline constexpr int NINDEX = -1;

/// End position.
inline constexpr int ENDPOS = -1;
...
public:
    ...
    void Insert(int pos, const Vector<T>& vector)
    {
        assert((pos >= 0 && pos <= size_) || pos == ENDPOS);
    ...
    void EraseSwap(int pos, int length = 1)
    {
        assert(pos >= 0 && length >= 1 && pos + length <= size_);

        int shiftStartIndex = pos + length;
        int newSize = size_ - length;
        int trailingCount = size_ - shiftStartIndex;
    ...
    void Resize(int newSize)
    {
        ...
        if constexpr (std::is_trivial<T>::value && std::is_standard_layout<T>::value)
        {
            if (newSize > capacity_)
            {
                ...
                unsigned char* newBuffer = AllocateBuffer((int)(capacity_ * sizeof(T)));
    ...
    void Reserve(int newCapacity)
    {
        ...
        if constexpr (std::is_trivial<T>::value && std::is_standard_layout<T>::value)
        {
            if (newCapacity != capacity_)
            {
                ...
                if (capacity_)
                {
                    newBuffer = AllocateBuffer((int)(capacity_ * sizeof(T)));
        ...
        else
        {
            if (newCapacity != capacity_)
            {
                ...
                if (capacity_)
                {
                    newBuffer = reinterpret_cast<T*>(AllocateBuffer((int)(capacity_ * sizeof(T))));
                    // Move the data into the new buffer
...
private:
    ...
    static void ConstructElements(T* dest, int count)
    {
        assert(count >= 0);
    ...
    static int CalculateCapacity(int size, int capacity)
    {
    ...
    Iterator DoInsertElements(int pos, RandomIteratorT start, RandomIteratorT end, Tag)
    {
        assert((pos >= 0 && pos <= size_) || pos == ENDPOS);
        ...
        const int numElements = (int)(end - start);
    ...
    Iterator DoEraseElements(int pos, int count)
    {
    ...
    static void CopyElements(T* dest, const T* src, int count)
    {
        assert(count >= 0);
