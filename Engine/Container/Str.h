...
public:
    ...
    {
        int rhsLength = CStringLength(rhs);
        Resize(rhsLength);
    ...
    /// Replace a substring.
    void Replace(int pos, int length, const String& replaceWith);
    /// Replace a substring with a C string.
    void Replace(int pos, int length, const char* replaceWith);
    ...
    String Substring(int pos, int length) const;
    ...
    int Find(const String& str, int startPos = 0, bool caseSensitive = true) const;
    /// Return index to the first occurrence of a character, or NPOS if not found.
    int Find(char c, int startPos = 0, bool caseSensitive = true) const;
    /// Return index to the last occurrence of a string, or NPOS if not found.
    int FindLast(const String& str, int startPos = NPOS, bool caseSensitive = true) const;
    /// Return index to the last occurrence of a character, or NPOS if not found.
    int FindLast(char c, int startPos = NPOS, bool caseSensitive = true) const;
    ...
    /// Return length.
    /// @property
    int Length() const { return IsShort() ? GetShortStringLength() : data_.longString_.length_; }

    /// Return buffer capacity.
    int Capacity() const { return IsShort() ? SHORT_STRING_CAPACITY : data_.longString_.capacity_; }
    ...
    static void EncodeUTF8(char*& dest, char32_t unicodeChar);
    /// Decode Unicode character from UTF8. Pointer will be incremented.
    static char32_t DecodeUTF8(const char*& src);
#ifdef _WIN32
    /// Encode Unicode character to UTF16. Pointer will be incremented.
    static void EncodeUTF16(wchar_t*& dest, char32_t unicodeChar);
    /// Decode Unicode character from UTF16. Pointer will be incremented.
    static char32_t DecodeUTF16(const wchar_t*& src);
#endif

    /// Return length of a C string.
    static int CStringLength(const char* str) { return str ? (int)strlen(str) : 0; }
    ...
    bool IsShort() const
    {
        unsigned char lastByte = data_.shortString_.length_;
...
private:
    ...
    inline static constexpr int SHORT_STRING_CAPACITY = sizeof(LongString) - 1;
    ...
    inline static constexpr unsigned char SSO_MASK = 0b10000000u;

    inline static constexpr unsigned char INVERTED_SSO_MASK = static_cast<unsigned char>(~SSO_MASK);

    int GetShortStringLength() const
    {
        return data_.shortString_.length_ & INVERTED_SSO_MASK;
    }

    void SetShortStringLength(unsigned char value)
    {
        data_.shortString_.length_ = value | SSO_MASK;
    }

    /// Move a range of characters within the string.
    void MoveRange(int dest, int src, int count)
    {
        if (count)
        {
    ...
    static void CopyChars(char* dest, const char* src, int count)
    {
#ifdef _MSC_VER
        if (count)
    ...
    void Replace(int pos, int length, const char* srcStart, int srcLength);
};
