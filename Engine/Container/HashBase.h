...
public:
    ...
    static inline constexpr int MIN_BUCKETS = 8;

    /// Maximum load factor.
    static inline constexpr int MAX_LOAD_FACTOR = 4;
    ...
    int Size() const { return ptrs_ ? (reinterpret_cast<int*>(ptrs_))[0] : 0; }

    /// Return number of buckets.
    int NumBuckets() const { return ptrs_ ? (reinterpret_cast<int*>(ptrs_))[1] : 0; }
...
protected:
    ...
    void AllocateBuckets(int size, int numBuckets);
