...
#define EDITOR_EXEC(action)					::Core::Global::ServiceLocator::Get<::Editor::Core::EditorActions>().action
#define EDITOR_BIND(method, ...)			std::bind(&Editor::Core::EditorActions::method, &::Core::Global::ServiceLocator::Get<::Editor::Core::EditorActions>(), ##__VA_ARGS__)
#define EDITOR_EVENT(target)				::Core::Global::ServiceLocator::Get<::Editor::Core::EditorActions>().target
#define EDITOR_CONTEXT(instance)			::Core::Global::ServiceLocator::Get<::Editor::Core::EditorActions>().GetContext().instance
#define EDITOR_RENDERER()					::Core::Global::ServiceLocator::Get<::Editor::Core::EditorActions>().GetRenderer()
#define EDITOR_PANEL(type, id)				::Core::Global::ServiceLocator::Get<::Editor::Core::EditorActions>().GetPanelsManager().GetPanelAs<type>(id)
...
namespace Editor::Core
{
	...
	public:
		...
		* @param p_parent
		*/
		template<typename T> ::Core::ECS::Actor& CreateMonoComponentActor(bool p_focusOnCreation = true, ::Core::ECS::Actor* p_parent = nullptr);

		/**
		* Calculate the position where to spawn the actor using the current camera position and forward
		* @param p_distanceToCamera
		*/
		Maths::FVector3 CalculateActorSpawnPoint(float p_distanceToCamera);
        ...
        * @param p_name
		*/
		::Core::ECS::Actor&	CreateEmptyActor(bool p_focusOnCreation = true, ::Core::ECS::Actor* p_parent = nullptr, const std::string& p_name = "");
        ...
        * @param p_name
		*/
		::Core::ECS::Actor&	CreateActorWithModel(const std::string& p_path, bool p_focusOnCreation = true, ::Core::ECS::Actor* p_parent = nullptr, const std::string& p_name = "");
		...
		* @param p_parent
		*/
		bool DestroyActor(::Core::ECS::Actor& p_actor);
		...
		* @param bool
		*/
		void DuplicateActor(::Core::ECS::Actor& p_toDuplicate, ::Core::ECS::Actor* p_forcedParent = nullptr, bool p_focus = true);
		#pragma endregion
		...
		* @param p_target
		*/
		void SelectActor(::Core::ECS::Actor& p_target);
		...
		*/
		::Core::ECS::Actor&		GetSelectedActor() const;
