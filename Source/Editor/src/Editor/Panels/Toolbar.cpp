...
) : PanelWindow(p_title, p_opened, p_windowSettings)
{
	...
	auto& textureManager = ::Core::Global::ServiceLocator::Get<::Core::ResourceManagement::TextureManager>();

	m_playButton	= &CreateWidget<UI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Play")->id, Maths::FVector2{ 20, 20 });
	m_pauseButton	= &CreateWidget<UI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Pause")->id, Maths::FVector2{ 20, 20 });
	m_stopButton	= &CreateWidget<UI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Stop")->id, Maths::FVector2{ 20, 20 });
	m_nextButton	= &CreateWidget<UI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Next")->id, Maths::FVector2{ 20, 20 });
	...
	{
		auto enable = [](UI::Widgets::Buttons::ButtonImage* p_button, bool p_enable)
		{
			p_button->disabled = !p_enable;
			p_button->tint = p_enable ? UI::Types::Color{ 1.0f, 1.0f, 1.0f, 1.0f} : UI::Types::Color{1.0f, 1.0f, 1.0f, 0.15f};
		};
