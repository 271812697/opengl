...
public:
	...
	{
		texture = Core::Global::ServiceLocator::Get<Core::ResourceManagement::TextureManager>()[p_path];
	}
			...
			generateMaterialsMenu.CreateWidget<UI::Widgets::Menu::MenuItem>("Standard").ClickedEvent += [this]
			{
				...
				if (auto model = modelManager.GetResource(resourcePath))
				{
					for (const std::string& materialName : model->GetMaterialNames())
					{
						...
						do
						{
							finalPath = Tools::Utils::PathParser::GetContainingFolder(filePath) + (!fails ? materialName : materialName + " (" + std::to_string(fails) + ')') + ".mat";
...
public:
	...
	virtual void CreateList() override
	{
		...
		editAction.ClickedEvent += [this]
		{
			Core::Resources::Material* material = OVSERVICE(Core::ResourceManagement::MaterialManager)[EDITOR_EXEC(GetResourcePath(filePath, m_protected))];
			if (material)
			{
				auto& materialEditor = EDITOR_PANEL(Editor::Panels::MaterialEditor, "Material Editor");
				...
				Core::Resources::Material* resource = Core::Global::ServiceLocator::Get<Core::ResourceManagement::MaterialManager>()[EDITOR_EXEC(GetResourcePath(filePath, m_protected))];
				auto& assetView = EDITOR_PANEL(Editor::Panels::AssetView, "Asset View");
				assetView.SetResource(resource);
				assetView.Open();
				assetView.Focus();
...
void Editor::Panels::AssetBrowser::ConsiderItem(UI::Widgets::Layout::TreeNode* p_root, const std::filesystem::directory_entry& p_entry, bool p_isEngineItem, bool p_autoOpen, bool p_scriptFolder)
{
	...
	itemGroup.CreateWidget<Visual::Image>(iconTextureID, Maths::FVector2{ 16, 16 }).lineBreak = false;
