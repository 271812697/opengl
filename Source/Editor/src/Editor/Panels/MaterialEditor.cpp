...
void DrawHybridVec3(UI::Internal::WidgetContainer& p_root, const std::string& p_name, Maths::FVector3& p_data, float p_step, float p_min, float p_max)
{
	Core::Helpers::GUIDrawer::CreateTitle(p_root, p_name);

	auto& rightSide = p_root.CreateWidget<UI::Widgets::Layout::Group>();

	auto& xyzWidget = rightSide.CreateWidget<UI::Widgets::Drags::DragMultipleScalars<float, 3>>(Core::Helpers::GUIDrawer::GetDataType<float>(), p_min, p_max, 0.f, p_step, "", Core::Helpers::GUIDrawer::GetFormat<float>());
	auto& xyzDispatcher = xyzWidget.AddPlugin<UI::Plugins::DataDispatcher<std::array<float, 3>>>();
	xyzDispatcher.RegisterReference(reinterpret_cast<std::array<float, 3>&>(p_data));
	xyzWidget.lineBreak = false;

	auto& rgbWidget = rightSide.CreateWidget<UI::Widgets::Selection::ColorEdit>(false, UI::Types::Color{ p_data.x, p_data.y, p_data.z });
	auto& rgbDispatcher = rgbWidget.AddPlugin<UI::Plugins::DataDispatcher<UI::Types::Color>>();
	rgbDispatcher.RegisterReference(reinterpret_cast<UI::Types::Color&>(p_data));
	rgbWidget.enabled = false;
	rgbWidget.lineBreak = false;

	auto& xyzButton = rightSide.CreateWidget<UI::Widgets::Buttons::Button>("XYZ");
	xyzButton.idleBackgroundColor = { 0.7f, 0.5f, 0.0f };
	xyzButton.lineBreak = false;

	auto& rgbButton = rightSide.CreateWidget<UI::Widgets::Buttons::Button>("RGB");
...
void DrawHybridVec4(UI::Internal::WidgetContainer& p_root, const std::string& p_name, Maths::FVector4& p_data, float p_step, float p_min, float p_max)
{
	Core::Helpers::GUIDrawer::CreateTitle(p_root, p_name);

	auto& rightSide = p_root.CreateWidget<UI::Widgets::Layout::Group>();

	auto& xyzWidget = rightSide.CreateWidget<UI::Widgets::Drags::DragMultipleScalars<float, 4>>(Core::Helpers::GUIDrawer::GetDataType<float>(), p_min, p_max, 0.f, p_step, "", Core::Helpers::GUIDrawer::GetFormat<float>());
	auto& xyzDispatcher = xyzWidget.AddPlugin<UI::Plugins::DataDispatcher<std::array<float, 4>>>();
	xyzDispatcher.RegisterReference(reinterpret_cast<std::array<float, 4>&>(p_data));
	xyzWidget.lineBreak = false;

	auto& rgbaWidget = rightSide.CreateWidget<UI::Widgets::Selection::ColorEdit>(true, UI::Types::Color{ p_data.x, p_data.y, p_data.z, p_data.w });
	auto& rgbaDispatcher = rgbaWidget.AddPlugin<UI::Plugins::DataDispatcher<UI::Types::Color>>();
	rgbaDispatcher.RegisterReference(reinterpret_cast<UI::Types::Color&>(p_data));
	rgbaWidget.enabled = false;
	rgbaWidget.lineBreak = false;

	auto& xyzwButton = rightSide.CreateWidget<UI::Widgets::Buttons::Button>("XYZW");
	xyzwButton.idleBackgroundColor = { 0.7f, 0.5f, 0.0f };
	xyzwButton.lineBreak = false;

	auto& rgbaButton = rightSide.CreateWidget<UI::Widgets::Buttons::Button>("RGBA");
...
void Editor::Panels::MaterialEditor::SetTarget(::Core::Resources::Material & p_newTarget)
{
...
Core::Resources::Material * Editor::Panels::MaterialEditor::GetTarget() const
{
	return m_target;
}
...
void Editor::Panels::MaterialEditor::CreateHeaderButtons()
{
	...
	{
		if (m_target)
			::Core::Resources::Loaders::MaterialLoader::Save(*m_target, EDITOR_EXEC(GetRealPath(m_target->path)));
	};
	...
	{
		if (m_target)
			::Core::Resources::Loaders::MaterialLoader::Reload(*m_target, EDITOR_EXEC(GetRealPath(m_target->path)));
...
{
	auto& columns = m_settings->CreateWidget<UI::Widgets::Layout::Columns<2>>();
	columns.widths[0] = 150;
...
void Editor::Panels::MaterialEditor::CreateMaterialSettings()
{
	...
	m_materialSettingsColumns = &m_materialSettings->CreateWidget<UI::Widgets::Layout::Columns<2>>();
...
void Editor::Panels::MaterialEditor::CreateShaderSettings()
{
	...
	m_shaderSettingsColumns = &m_shaderSettings->CreateWidget<UI::Widgets::Layout::Columns<2>>();
...
void Editor::Panels::MaterialEditor::GenerateShaderSettingsContent()
{
	...
	for (auto&[name, value] : m_target->GetUniformsData())
	{
		...
		if (uniformData)
		{
            ...
            case UniformType::UNIFORM_UINT:			orderID = 6; break;
			case UniformType::UNIFORM_BOOL:			orderID = 7; break;
			}
	...
	for (auto& [order, info] : sortedUniformsData)
	{
		...
		if (uniformData)
		{
            ...
            case UniformType::UNIFORM_UINT:			GUIDrawer::DrawScalar<unsigned int>(*m_shaderSettingsColumns, UniformFormat(info.first), reinterpret_cast<unsigned int&>(*info.second));																break;
			...
			case UniformType::UNIFORM_FLOAT_VEC2:	GUIDrawer::DrawVec2(*m_shaderSettingsColumns, UniformFormat(info.first), reinterpret_cast<Maths::FVector2&>(*info.second), 0.01f, GUIDrawer::_MIN_FLOAT, GUIDrawer::_MAX_FLOAT);	break;
			case UniformType::UNIFORM_FLOAT_VEC3:	DrawHybridVec3(*m_shaderSettingsColumns, UniformFormat(info.first), reinterpret_cast<Maths::FVector3&>(*info.second), 0.01f, GUIDrawer::_MIN_FLOAT, GUIDrawer::_MAX_FLOAT);			break;
			case UniformType::UNIFORM_FLOAT_VEC4:	DrawHybridVec4(*m_shaderSettingsColumns, UniformFormat(info.first), reinterpret_cast<Maths::FVector4&>(*info.second), 0.01f, GUIDrawer::_MIN_FLOAT, GUIDrawer::_MAX_FLOAT);			break;
...
void Editor::Panels::MaterialEditor::GenerateMaterialSettingsContent()
{
	...
	GUIDrawer::DrawBoolean(*m_materialSettingsColumns, "Blendable", std::bind(&::Core::Resources::Material::IsBlendable, m_target), std::bind(&::Core::Resources::Material::SetBlendable, m_target, std::placeholders::_1));
	GUIDrawer::DrawBoolean(*m_materialSettingsColumns, "Back-face Culling", std::bind(&::Core::Resources::Material::HasBackfaceCulling, m_target), std::bind(&::Core::Resources::Material::SetBackfaceCulling, m_target, std::placeholders::_1));
	GUIDrawer::DrawBoolean(*m_materialSettingsColumns, "Front-face Culling", std::bind(&::Core::Resources::Material::HasFrontfaceCulling, m_target), std::bind(&::Core::Resources::Material::SetFrontfaceCulling, m_target, std::placeholders::_1));
	GUIDrawer::DrawBoolean(*m_materialSettingsColumns, "Depth Test", std::bind(&::Core::Resources::Material::HasDepthTest, m_target), std::bind(&::Core::Resources::Material::SetDepthTest, m_target, std::placeholders::_1));
	GUIDrawer::DrawBoolean(*m_materialSettingsColumns, "Depth Writing", std::bind(&::Core::Resources::Material::HasDepthWriting, m_target), std::bind(&::Core::Resources::Material::SetDepthWriting, m_target, std::placeholders::_1));
	GUIDrawer::DrawBoolean(*m_materialSettingsColumns, "Color Writing", std::bind(&::Core::Resources::Material::HasColorWriting, m_target), std::bind(&::Core::Resources::Material::SetColorWriting, m_target, std::placeholders::_1));
	GUIDrawer::DrawScalar<int>(*m_materialSettingsColumns, "GPU Instances", std::bind(&::Core::Resources::Material::GetGPUInstances, m_target), std::bind(&::Core::Resources::Material::SetGPUInstances, m_target, std::placeholders::_1), 1.0f, 0, 100000);
}
