...
template<class T>
std::function<void()> ActorWithComponentCreationHandler(Core::ECS::Actor* p_parent, std::optional<std::function<void()>> p_onItemClicked)
{
    return Combine(EDITOR_BIND(CreateMonoComponentActor<T>, true, p_parent), p_onItemClicked);
}

std::function<void()> ActorWithModelComponentCreationHandler(Core::ECS::Actor* p_parent, const std::string& p_modelName, std::optional<std::function<void()>> p_onItemClicked)
{
    return Combine(EDITOR_BIND(CreateActorWithModel, ":Models\\" + p_modelName + ".fbx", true, p_parent, p_modelName), p_onItemClicked);
}

void Editor::Utils::ActorCreationMenu::GenerateActorCreationMenu(UI::Widgets::Menu::MenuList& p_menuList, ::Core::ECS::Actor* p_parent, std::optional<std::function<void()>> p_onItemClicked)
{
