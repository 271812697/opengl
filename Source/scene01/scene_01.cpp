...
int main(int, char**) {
    {
        ...
        windowSettings.title = "Tiled Forward Renderer";
        ...
        //初始化场景
        ::core::Log::Init();
        scene::Scene* S = new scene::Scene01("Tiled Forward Renderer");
        S->Init();
        S->Resize(1600, 900);
        //初始化UI
...
namespace scene {
    ...
    void Scene01::Init() {
        ...
        resource_manager.Add(04, MakeAsset<Shader>( "res\\shaders\\pbr01.glsl"));
        resource_manager.Add(05, MakeAsset<Shader>("res\\shaders\\post_process01.glsl"));
        resource_manager.Add(00, MakeAsset<CShader>("res\\shaders\\bloom.glsl"));
        resource_manager.Add(10, MakeAsset<CShader>("res\\shaders\\cull.glsl"));
    ...
    void Scene01::OnImGuiRender(float dt ) {
        ...
        if (  BeginTabBar("InspectorTab", ImGuiTabBarFlags_None)) {
            Indent(5.0f);
    ...
    void Scene01::SetupPLBuffers() {
        ...
        nx = (this->width  + tile_size - 1) / tile_size;
        ny = (this->height + tile_size - 1) / tile_size;
