...
// PCF 算法 以一致的采样半径去过渡阴影边界
float PCF(vec4 fragPosLightSpace, float radius) {
    // 采样距离修正
    radius *= pl.PCF_SampleRadius;
    vec3 projCoords = fragPosLightSpace.xyz / fragPosLightSpace.w;
    projCoords = projCoords * 0.5 + 0.5;
    if(projCoords.x<0||projCoords.x>1||projCoords.y<0||projCoords.y>1)return 0.0;
    float shadow = 0.0;
...
//PCSS 是PCF的改进，通过估计遮挡和自身深度 去计算PCF采样半径
...
float findBlocker(vec2 uv, float zReceiver) {
    int blockers = 0;
    float ret = 0.0;
    float r = pl.lightWidth * (zReceiver - NEAR_PLANE/FAR_PLANE) / zReceiver;
    // 弥散参数修正
    r *= pl.SMDiffuse;
    vec2 texelSize = 1.0 / textureSize(depthMap, 0); // 0 级纹理相邻纹素的距离
...
float PCSS(vec4 fragPosLightSpace){
    ...
    float penumbra = (depth - avgDepth) / avgDepth * pl.lightWidth;
